...
/**
 * Ћогическа€ часть клинта
 * ќтвечает за обработку сообщений от пользовател€
 * @author ataryq
 *
 */
public class Client  implements InterfaceClient, InterfaceAction {
	...
	/**
	 * ”становить текущее состо€ние обработчика
	 * @param _state номер состо€ни€
	 */
		...
		case "1": {
			...
			if(msgs[1].length() < 4) {
				...
				if( control.RegistredNewUser(msgs[1], this) ) {
					...
					//уведомить клиент о том, что логин уже зан€т
	...
	/**
	 * InMenu state
	 * —осто€ние активно когда игрок в меню
	...
	protected void InMenu(String msg) {
		...
		switch(msgs[0])
		{
		//запрос на получени€ списка игроков
		case "1": {
			String find_str = "";
			if(msgs.length > 1) {
	...
	/**
	 * —осто€ние игры
	 * јктивно когда польз. с кем то играет
	 * @param msg
	 */
	...
	/**
	 * ќтправл€ет информацию об изменении камней на доске(удаление пленников и прочее)
	...
	/**
	 * ѕосылает колличесто пленников и обоих игроков
	 */
